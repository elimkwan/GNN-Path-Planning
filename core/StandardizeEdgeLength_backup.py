...
# Working Version, can aggregate short edge and divide long edge
...
def popShortEdges(start_node, mid_nodes, end_node, n_list, e_list, graph):
    ...
    return node_list, edge_list, graph, removed_edges
...
#             if elem[0] == mid_node or elem[1] == mid_node:
#                 edge_list.remove(elem)
#                 removed_edges.append(elem)
#         graph.remove_node(mid_node)
...
def standardizeEdgeLength(nodes, edges, lengths):
    ...
    for e in short_edges:
            ...
            for cur_node in [start_node, end_node]:
                ...
                for n in new_G.neighbors(cur_node):
                    ...
                    if n == end_node or n == start_node or n in mid_nodes \
                        or new_nodes[n] == Point(VOID, VOID)\
                        or not correct_num_of_neighbors \
                        or [n, cur_node] in sorted_edge \
                        or [cur_node, n] in sorted_edge:
                        prev_start_end_node = (start_node, end_node)
                        if loop > 5:
                            stopping = True
                        continue
        ...
        if num_edges_formed > 0 and len(mid_nodes) > 0:
            new_nodes, new_edges, new_G, removed_edges = popShortEdges(
                start_node, 
                mid_nodes, 
                end_node, 
                new_nodes, 
                new_edges,
                new_G)
            sorted_edge.append(removed_edges)
