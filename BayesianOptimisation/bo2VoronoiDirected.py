...
def bo2_voronoi_directed(exp, num_sample = 100):
    ...
    domain.append({
                'name':  k+3, 
                'type': 'continuous', 
                'domain': (0, constant.MAX_SUBGRAPH)
                })
    ...
    opt = BayesianOptimization(f = simulateObj.run_simulator, maximize=False, \
                                ...
                                kernel=kern, acquisition_type='EI', verbosity=True)
...
def finalRun(
    ...
    if subgraph_thres < 0.000001:
        ...
        if np.array(paths).size == exp.NUM_OF_AGENT and np.all(np.array(paths).flatten() == [None]*exp.NUM_OF_AGENT):
            ...
            paths = np.array(start_nodes).reshape((len(start_nodes), -1))
        return paths, cost, G, directed_voronoi, subgraph_thres

    else:
        print("Use CBS with Subgraph")
...
def get_results(opt_probabilities, exp):
    ...
    if subgraph_thres < 0.0001:
        print("update env graph to use initial graph")
        env.G = init_graph
